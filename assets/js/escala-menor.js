...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var TIPOS = {
    natural: { label: "Natural", fn: TM.escalaMenor },
    harmonica: { label: "Harm\u00f4nica", fn: TM.escalaMenorHarmonica },
    melodica: { label: "Mel\u00f3dica", fn: TM.escalaMenorMelodica },
  };
  ...
  function renderEscala(tonica, grauAtivo, tipo) {
    var config = TIPOS[tipo];
    if (!config) return "";
    var escala = config.fn(tonica);
    if (!escala) return "";
    var cifra = TM.notaParaCifra(tonica) + "m";
  ...
  function renderEmpty(container) {
    container.innerHTML =
      '<div class="teoria-tool-card__result-empty">Selecione o tipo e a escala</div>';
  }

  function calcular(lado) {
    var tipo = document.getElementById(lado + "-tipo").value;
    ...
    if (mode === "consulta") {
      container.innerHTML = renderEscala(tonica, -1, tipo);
      return;
    }
    ...
    EX.renderExercicio({
      ...
      pergunta: "de " + tonica + " menor " + config.label,
      placeholder: "Ex: F\u00e1#",
      lado: lado,
      correto: escala[grau],
  ...
  function limpar(lado) {
    document.getElementById(lado + "-tipo").value = "";
