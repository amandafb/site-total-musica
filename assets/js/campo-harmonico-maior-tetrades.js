...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCampoHarmonico(tonica, grauAtivo) {
    var acordes = TM.campoHarmonico(tonica, "tetrades");
    if (!acordes) return "";
  ...
  function calcular(lado) {
    ...
    var acorde = TM.acordeTetrade(tonica, grau);
    if (!acorde) return;

    EX.renderExercicioCampo({
      container: container,
      destaque: TM.GRAUS_ROMANOS[grau] + " grau",
      pergunta: "de " + tonica + " Maior (t\u00e9trade)",
      placeholderCifra: "Ex: Dm7",
      placeholderFormacao: "Ex: R\u00e9 F\u00e1 L\u00e1 D\u00f3",
      lado: lado,
