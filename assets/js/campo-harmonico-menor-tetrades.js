...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCampoHarmonico(tonica, grauAtivo) {
    var acordes = TM.campoHarmonicoMenor(tonica, "tetrades");
    if (!acordes) return "";
  ...
  function calcular(lado) {
    ...
    var acorde = TM.acordeTetradeMenor(tonica, grau);
    if (!acorde) return;

    EX.renderExercicioCampo({
      container: container,
      destaque: TM.GRAUS_ROMANOS_MENOR[grau] + " grau",
      pergunta: "de " + tonica + " menor harm\u00f4nico (t\u00e9trade)",
      placeholderCifra: "Ex: Am(7M)",
      placeholderFormacao: "Ex: L\u00e1 D\u00f3 Mi Sol#",
      lado: lado,
